...
class RandomResizeLongForSegmetation():

    def __init__(self, min_long, max_long):
        self.min_long = min_long
        self.max_long = max_long

    def __call__(self, data, sal=None):

        imgarr, maskarr = data['image'], data['mask']
        target_long = random.randint(self.min_long, self.max_long)
        w, h = imgarr.size
        ...
        img = imgarr.resize(target_shape, resample=PIL.Image.CUBIC)
        mask = maskarr.resize(target_shape, resample=PIL.Image.NEAREST)
        if sal:
           sal = sal.resize(target_shape, resample=PIL.Image.CUBIC)
           return img, sal
        data['image'] = img
...
class RandomCropForSegmentation(RandomCrop):

    def __init__(self, cropsize):
        super(RandomCropForSegmentation, self).__init__(cropsize)
    ...
    def __call__(self, data, sal=None):
        ...
        container = np.zeros((self.cropsize, self.cropsize, imgarr.shape[-1]), np.float32)
        container_mask = np.zeros((self.cropsize, self.cropsize), maskarr.dtype)
        container[cont_top:cont_top+ch, cont_left:cont_left+cw] = \
            imgarr[img_top:img_top+ch, img_left:img_left+cw]
        container_mask[cont_top:cont_top + ch, cont_left:cont_left + cw] = \
            maskarr[img_top:img_top + ch, img_left:img_left + cw]
        if sal is not None:
            container_sal = np.zeros((self.cropsize, self.cropsize,1), np.float32)
            container_sal[cont_top:cont_top+ch, cont_left:cont_left+cw,0] = \
                sal[img_top:img_top+ch, img_left:img_left+cw]
