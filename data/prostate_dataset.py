...
def load_label_list(dataset_index):
    ...
    for img in img_gt_name_list:
        ...
        cat_names = img.split(' ')[1:]
        for cat_name in cat_names:
            if cat_name in CAT_LIST:
                cat_num = CAT_NAME_TO_NUM[cat_name]
                multi_cls_lab[cat_num] = 1.0

        gt_list.append(multi_cls_lab)
    return gt_list


def get_img_path(img_name, data_root):
    return os.path.join(data_root, 'DL_Image', img_name + '.png')
...
class ProstateImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = PIL.Image.open(os.path.join(self.data_root, 'DL_Image', name + '.png'))
...
class ProstateClsDataset(ProstateImageDataset):
...
class ProstateClsDatasetMSF(ProstateClsDataset):
...
class ProstateClsDatasetMS(ProstateClsDataset):
...
class ProstateAffDataset(ProstateImageDataset):
...
class ProstateAffGtDataset(ProstateImageDataset):
...
class ProstateDatasetForTestingCAM(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # img = PIL.Image.open(get_img_path(name, self.data_root)).convert("RGB")
        img = PIL.Image.open(os.path.join(self.data_root, 'DL_Image', name + '.png'))
        mask = PIL.Image.open(os.path.join(self.data_root, 'DL_Label', name.replace('Image', 'Label') + '.png'))
        label = torch.from_numpy(self.label_list[idx])

        if self.transform is not None:
            input_dic = {'image': img, 'mask': mask}
            output_dic = self.transform(input_dic)
            img = output_dic['image']
